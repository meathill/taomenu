...
- [ ] Pro 权益预览、AI 输入/输出/限制说明与应用内升级入口
- [ ] 越南语语音录入菜单草稿，明确浏览器兼容性并要求手工确认
- [ ] AI 菜单翻译：选择目标语言、异步生成、逐项审核、写入草稿
- [ ] AI 菜品图片美化：保留原图、生成预览、确认替换、可撤销
- [ ] 每个阶段补测试，运行 format / typecheck / test / build
- [ ] 每个可独立验收阶段 commit + push，并等待自动部署后复验
- [ ] 更新 `TODO.md` 与 GitHub issue #1，只勾选已完成且经生产验收的项目
